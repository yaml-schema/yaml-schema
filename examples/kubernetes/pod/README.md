...
`examples/kubernetes/deployment/schema.yaml` via a cross-file `$ref`.
...
```sh
cargo run --bin ys -- -f examples/kubernetes/pod/schema.yaml examples/kubernetes/pod/fixtures/simple-pod.yaml
```

Or validate all of them at once:

```sh
cargo run --bin ys -- -f examples/kubernetes/pod/schema.yaml examples/kubernetes/pod/fixtures/*.yaml
```
