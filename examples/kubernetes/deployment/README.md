...
[`examples/kubernetes/pod/schema.yaml`](../pod/schema.yaml) via a cross-file `$ref`
...
```sh
cargo run --bin ys -- -f examples/kubernetes/deployment/schema.yaml examples/kubernetes/deployment/fixtures/simple-deployment.yaml
```

Or validate all of them at once:

```sh
cargo run --bin ys -- -f examples/kubernetes/deployment/schema.yaml examples/kubernetes/deployment/fixtures/*.yaml
```
